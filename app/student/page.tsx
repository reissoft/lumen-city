...
import { Play, Map, Trophy, Star, LogOut } from "lucide-react"
import Image from "next/image"
import { cookies } from "next/headers" // Importe isso
import { redirect } from "next/navigation" // Importe isso
...
async function getStudentData() {
  ...
  // Buscar atividades disponíveis (Hardcoded teacher para MVP)
...
export default async function StudentHub() {
  ...
  // Cálculo fictício de nível (Ex: cada nível precisa de 100 XP)
    ...
    <Button variant="outline" size="sm" className="gap-2 text-red-500 hover:text-red-700 hover:bg-red-50 border-red-200">
      <LogOut size={16} /> Sair
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* CARTÃO DA CIDADE */}
          <Link href="/student/city" className="group">
            <Card className="h-full bg-gradient-to-br from-indigo-600 to-purple-700 text-white border-none hover:shadow-xl transition-all hover:scale-[1.02] cursor-pointer relative overflow-hidden">
              ...
              <CardContent>
                <p className="text-indigo-100 mb-6">
                  Gerencie seus prédios, colete recursos e expanda seu império.
                </p>
          ...
          {/* CARTÃO DE RANKING (Placeholder) */}
          <Card className="bg-white hover:shadow-md transition-shadow">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-slate-800">
                <Trophy className="text-yellow-500" /> Ranking da Turma
              </CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
            <Star className="text-orange-500" fill="currentColor" /> Missões Disponíveis
          </h2>
          
          <div className="grid gap-4 md:grid-cols-3">
            {activities.map((activity) => (
              <Card key={activity.id} className="hover:border-indigo-300 transition-colors">
                <CardHeader className="pb-2">
                  <div className="flex justify-between">
                    <Badge variant="outline" className="text-xs">{activity.type}</Badge>
                    <span className="text-xs text-slate-400">Dif. {activity.difficulty}</span>
                  </div>
